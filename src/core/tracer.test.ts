...
describe('createTracer', () => {
  ...
  it('records duration', async () => {
    ...
    await trace('test', async () => { clock.advance(50) })
    assert.equal(writer.allEvents()[1].duration, 50)
  })

  it('returns callback result', async () => {
    const { trace } = createTracer({ writer: createMemoryWriter(), idGenerator: createSeqId(), clock: createFakeClock() })
  ...
  it('records error and re-throws', async () => {
    ...
    await assert.rejects(
      () => trace('fail', async () => { throw new Error('boom') }),
      { message: 'boom' }
    )
