// src/readers/tree-builder.ts
import type { TraceEvent, TraceTreeNode, SerializedError } from '../types.ts'
...
export function buildTree(events: TraceEvent[]): TraceTreeNode {
  ...
  for (const event of events) {
    switch (event.type) {
      case 'trace:start': {
        root = { type: 'trace', id: event.id as string, name: event.name as string, status: 'in_progress', ts: event.ts, children: [] }
      ...
      case 'span:start': {
        const node: TraceTreeNode = { type: 'span', id: event.id as string, name: event.name as string, status: 'in_progress', ts: event.ts, children: [] }
      ...
      case 'checkpoint': {
        const node: TraceTreeNode = { type: 'checkpoint', name: event.name as string, ts: event.ts, data: event.data, children: [] }
        const parent = nodeMap.get(event.parent as string)
        if (parent) parent.children.push(node)
        break
      }
